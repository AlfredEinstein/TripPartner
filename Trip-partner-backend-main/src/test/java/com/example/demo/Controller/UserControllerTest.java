...
class UserControllerTest {
    ...
    void testChangePassword() {
        ...
        when(userService.changePassword(userId, userPassword,newpassword)).thenReturn(expectedResponse);

        ResponseEntity<String> actualResponse = userController.changePassword(userId, userPassword,newpassword);

        assertEquals(HttpStatus.OK, actualResponse.getStatusCode());
        assertEquals("Password changed successfully", actualResponse.getBody());
        verify(userService, times(1)).changePassword(userId, userPassword,newpassword);
    }
